...
```bash
kubectl -n thingsboard-mqtt-broker scale statefulset tbmq --replicas=0
```
...
1. **Apply the Backup Pod YAML**:
   ```bash
   kubectl -n thingsboard-mqtt-broker apply -f backup-restore/backup.yml
   ```
...
3. **View Logs to Confirm Backup Completion**:
   ...
   ```bash
   kubectl -n thingsboard-mqtt-broker logs postgres-backup
   ```

4. **Clean Up the Backup Pod** (Optional):
   After completion, you can delete the backup pod if needed:
   ```bash
   kubectl -n thingsboard-mqtt-broker delete pod postgres-backup
   ```
...
1. **Apply the Restore Pod YAML**:
   ```bash
   kubectl -n thingsboard-mqtt-broker apply -f backup-restore/restore.yml
   ```
...
3. **View Logs to Confirm Restore Completion**:
   ...
   ```bash
   kubectl -n thingsboard-mqtt-broker logs postgres-restore
   ```
...
5. **Clean Up the Restore Pod** (Optional):
   ...
   ```bash
   kubectl -n thingsboard-mqtt-broker delete pod postgres-restore
   ```
...
```bash
kubectl -n thingsboard-mqtt-broker scale statefulset tbmq --replicas=2
```
